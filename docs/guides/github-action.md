...
jobs:
  check:
    ...
    steps:
      ...
      - uses: complydoc/complydoc@v0.4.12
        with:
          path: documents
          policy: policy.yaml
          sarif: true
...
| `version` | the tag | complydoc to install. Empty installs the version in the tag the action was used at (`@v0.4.12` installs 0.4.12), or the latest release for any other ref |
