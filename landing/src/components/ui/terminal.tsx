...
interface TerminalProps {
  children: React.ReactNode
  className?: string
  /** complydoc: a label in the title bar, such as the working directory. */
  title?: string
  sequence?: boolean
  startOnView?: boolean
}
...
export const Terminal = ({
  ...
  const content = (
    <div
      ref={containerRef}
      className={cn(
        "border-border bg-card z-0 h-full w-full rounded-xl border",
        className
      )}
    >
      <div className="border-border flex items-center gap-x-2 border-b px-4 py-3">
        {/* complydoc: the window dots take the theme's state colours, not raw ones. */}
        <div className="flex flex-row gap-x-2">
          <div className="h-2 w-2 rounded-full bg-destructive"></div>
          <div className="h-2 w-2 rounded-full bg-warning"></div>
          <div className="h-2 w-2 rounded-full bg-success"></div>
        </div>
        {title && <span className="text-muted-foreground ml-2 truncate font-mono text-xs">{title}</span>}
      </div>
