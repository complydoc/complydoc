...
class Judge(Service):
    """Stands in for api.typesafe.ai for the concept judge: answers each question it is asked."""

    def handle(self, request: httpx2.Request) -> httpx2.Response:
        self.requests.append(request)
        asked = json.loads(request.content).get("questions", {})
        return httpx2.Response(
            200,
            json={
                "model": "jev-1",
                "answers": {name: {"type": "noul", "noul": self.score} for name in asked},
                "usage": {"input_tokens": 40, "output_tokens": 1},
                "request_id": "req-2",
            },
        )
...
def test_the_concept_judge_asks_in_the_concepts_own_words(monkeypatch):
    from complydoc.integrations.typesafe import jev_concept_judge

    monkeypatch.setenv("JEV_KEY", "not-a-real-key")
    service = Judge(score=0.83)
    judge = jev_concept_judge(allow_network=True, http_client=service.client(), max_characters=20)

    score = judge(
        "Renewal quote", "A quote offering to renew a policy.", "We offer renewal at 420 a year."
    )
    assert score == pytest.approx(0.83)
    body = service.body
    assert body["state"] == {"page": "We offer renewal at "}, "the page is capped before it is sent"
    (question,) = body["questions"].values()
    assert "Renewal quote" in json.dumps(question)
    assert "A quote offering to renew a policy." in json.dumps(question)
    assert judge("Renewal quote", "A quote offering to renew a policy.", "   ") == 0.0
    assert len(service.requests) == 1, "a blank page is not sent"


def test_the_concept_judge_refuses_unless_the_caller_allowed_the_network():
    from complydoc.integrations.typesafe import jev_concept_judge
